...
void sprites_load_player(gpointer user_data) {
    ...
    // Load player sprite
    game_state->player.animation.sprite_sheet = cairo_image_surface_create_from_png("./assets/mario_sprite_sheet.png");
    if (cairo_surface_status(game_state->player.animation.sprite_sheet) != CAIRO_STATUS_SUCCESS) {
    ...
    update_animation_frame(&player->animation);
    if (game_state->player.animation.current_frame == NULL || 
        cairo_surface_status(game_state->player.animation.current_frame) != CAIRO_STATUS_SUCCESS) {
...
void sprites_cleanup(game_state_t *game_state) {
    if (game_state->player.animation.current_frame != NULL) {
        cairo_surface_destroy(game_state->player.animation.current_frame);
        game_state->player.animation.current_frame = NULL;
    }
    
    if (game_state->player.animation.sprite_sheet != NULL) {
        cairo_surface_destroy(game_state->player.animation.sprite_sheet);
        game_state->player.animation.sprite_sheet = NULL;
    }
