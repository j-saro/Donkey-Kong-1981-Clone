...
void sprites_load_level(gpointer user_data) {
    ...
    game_state->level.sprite_sheet = cairo_image_surface_create_from_png("./assets/platform.png");
    if (cairo_surface_status(game_state->level.sprite_sheet) != CAIRO_STATUS_SUCCESS) {
        g_warning("Failed to create level spritesheet");
        sprites_cleanup(game_state);
        return;
    }
}
...
void sprites_cleanup(game_state_t *game_state) {
    ...
    if (game_state->level.sprite_sheet != NULL) {
        cairo_surface_destroy(game_state->level.sprite_sheet);
        game_state->level.sprite_sheet = NULL;
    }
}